...
extern int speed;*/
int indx = 0, indy = 0;
extern struct Encoder Encoder1, Encoder2;
extern struct Odom theOdom;
int32_t prev_Enc1_pos, prev_Enc2_pos = 0;
...
void SysTick_Handler(void)
{
  /* USER CODE BEGIN SysTick_IRQn 0 */
	...
	if(indx == period){
		...
		Encoder1.speed = (Encoder1.position - prev_Enc1_pos)*1000/(period);
		Speed_Publish(&Encoder1);
		//int32_msg_3.data = Encoder1.speed;
		//enkoder1_vel_pub.publish(&int32_msg_3);
		prev_Enc1_pos = Encoder1.position;

		Encoder2.speed = (Encoder2.position - prev_Enc2_pos)*1000/(period);
		Speed_Publish(&Encoder2);
		//int32_msg_4.data = Encoder2.speed;
		//enkoder2_vel_pub.publish(&int32_msg_4);
		prev_Enc2_pos = Encoder2.position;
		//speed = (position - oldpos)*1000/(period); //ms to s
		//oldpos = position;
		indx = 0;
